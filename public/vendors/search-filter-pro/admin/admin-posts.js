(function ($) {
  ...
  $(function () {
    ...
    jQuery.fn.SfPopupTax = function (options) {
      ...
      $(this).each(function () {
        function getTaxonomyTerms(taxonomy_name, taxonomy_ids, $target) {
          ...
          $.get(ajaxurl + '?action=' + action_name + '&taxonomy_name=' + taxonomy_name + '&taxonomy_ids=' + taxonomy_ids).done(function (data) { //don't do anything
    ...
    String.prototype.parseArray = function (arr) {
      return this.replace(/\{([0-9]+)\}/g, function (_, index) {
        return arr[index];
      });
    };
    ...
    jQuery.fn.makeWidget = function (options) {
      ...
      this.each(function () {
        ...
        item.find('.widget-top').click(function (e) {
          ...
          if (typeof (allowExpand) == 'undefined') { //init data-dragging if its not on already
          ...
          if (allowExpand == 1) {
            ...
            if (typeof (dataDragging) == 'undefined') { //init data-dragging if its not on already
        ...
        function set_meta_widget_title(item) {
          ...
          } else if (radio_val == 'number') { //then there is only 1 meta key
    ...
    jQuery.fn.makeSortables = function (options) {
      ...
      this.each(function () {
        ...
        container.click(function () { //prevent animation when the container is closed - no need to animate the helper to an invisible DIV.....
